...
#pragma omp parallel default(none) shared(n_threads) shared(next_priv) shared(cur) shared(p)
        {
            int index = omp_get_thread_num();
            ...
            while(rem_red > 1) {
                int next_index = index + red_procs;
                if (index < red_procs && next_index < rem_red) {
                    if (LEFT == turn(p[cur], p[next_priv[index]], p[next_priv[next_index]])) {
                        next_priv[index] = next_priv[next_index];
                    }
                }
#pragma omp barrier
