...
void convex_hull_partial(const points_t *pset, points_t *hull, point_t start, point_t end) {
    /* Computes partial hull from start to end TODO */
}
...
void convex_hull(const points_t *pset, points_t *hull)
{
    ...
    int i, j;
    int cur, next, leftmost, rightmost;

    points_t pset_upper, pset_lower;
    points_t hull_upper, hull_lower;
    ...
    /* Identify the leftmost-lower and rightmost-upper points p[leftmost], p[rightmost] */
    leftmost = 0;
    rightmost = 0;
    for (i = 1; i<n; i++) {
        ...
        if (p[i].x > p[rightmost].x) {
            rightmost = i;
        } else if (p[i].x == p[rightmost].x && p[i].y > p[rightmost].y) {
            rightmost = i;
        }
    }
    
    pset_upper.p = (point_t*)malloc(n * sizeof(point_t));
    pset_lower.p = (point_t*)malloc(n * sizeof(point_t)); 

    /* Divide upper and lower domains */
    for (i = 0; i < n; i++) {
        if (i == leftmost || i == rightmost) {
            pset_upper.p[pset_upper.n++] = p[i];
            pset_lower.p[pset_lower.n++] = p[i];
        } else {
            int res = turn(p[leftmost], p[rightmost], p[i]);
            if (res == LEFT) {
                pset_upper.p[pset_upper.n++] = p[i];
            } else if (res == RIGHT) {
                pset_lower.p[pset_lower.n++] = p[i];
    ...
    /* Calculate upper and lower hull separately */
#pragma omp parallel num_threads(2)
    {
#pragma omp sections
        {
#pragma omp section
            convex_hull_partial(pset_upper, hull_upper, p[leftmost], p[rightmost]);
#pragma omp section
            convex_hull_partial(pset_lower, hull_lower, p[rightmost], p[leftmost]);
        }
    }
