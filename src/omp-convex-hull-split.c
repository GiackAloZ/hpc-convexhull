...
/* Computes a partial hull with the Gift Wrapping algorithm,
considering the point at index 0 as the point to start with the algorithm. */
void partial_convex_hull(const points_t *pset, points_t *hull){
    const int n = pset->n;
    const point_t *p = pset->p;
    int i, j;
    int cur, next, start, end;
...
void convex_hull(const points_t *pset, points_t *hull)
{
    ...
    for (i = 1; i<n; i++) {
        if (p[i].x < p[leftmost].x || (p[i].x == p[leftmost].x && p[i].y < p[leftmost].y)) {
            leftmost = i;
        } else if (p[i].x > p[rightmost].x || (p[i].x == p[rightmost].x && p[i].y > p[rightmost].y)) {
            rightmost = i;
        }
    }    
    ...
    partial_convex_hull(&upper_set, &upper_hull);
    partial_convex_hull(&lower_set, &lower_hull);
